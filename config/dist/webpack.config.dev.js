...
var reactRefreshOverlayEntry = require.resolve('react-dev-utils/refreshOverlayInterop'); // Some apps do not need the benefits of saving a web request, so not inlining the chunk
// makes for a smoother build process.


var shouldInlineRuntimeChunk = process.env.INLINE_RUNTIME_CHUNK !== 'false';
var emitErrorsAsWarnings = process.env.ESLINT_NO_DEV_ERRORS === 'true';
var disableESLintPlugin = process.env.DISABLE_ESLINT_PLUGIN === 'true';
var imageInlineSizeLimit = parseInt(process.env.IMAGE_INLINE_SIZE_LIMIT || '10000'); // Check if TypeScript is setup

var useTypeScript = fs.existsSync(paths.appTsConfig); // Get the path to the uncompiled service worker (if it exists).

var swSrc = paths.swSrc; // style files regexes
...
module.exports = function (webpackEnv) {
  ...
  var isEnvProduction = webpackEnv === 'production'; // Variable used for enabling profiling in Production
  // passed into alias object. Uses a flag if passed into the build command

  var isEnvProductionProfile = isEnvProduction && process.argv.includes('--profile'); // We will provide `paths.publicUrlOrPath` to our app
  // as %PUBLIC_URL% in `index.html` and `process.env.PUBLIC_URL` in JavaScript.
  // Omit trailing slash as %PUBLIC_URL%/xyz looks better than %PUBLIC_URL%xyz.
  // Get environment variables to inject into our app.

  var env = getClientEnvironment(paths.publicUrlOrPath.slice(0, -1));
  var shouldUseReactRefresh = env.raw.FAST_REFRESH; // common function to get style loaders
  ...
  return {
    ...
    entry: isEnvDevelopment && !shouldUseReactRefresh ? [// Include an alternative client for WebpackDevServer. A client's job is to
      ...
      rules: [// Disable require.ensure as it's not a standard language feature.
      {
        ...
        // The preset includes JSX, Flow, TypeScript, and some ESnext features.
        {
          ...
          options: {
            ...
            }], isEnvDevelopment && shouldUseReactRefresh && require.resolve('react-refresh/babel')].filter(Boolean),
    ...
    new webpack.DefinePlugin(env.stringified), // This is necessary to emit hot updates (CSS and Fast Refresh):
    isEnvDevelopment && new webpack.HotModuleReplacementPlugin(), // Experimental hot reloading for React .
    ...
    new ManifestPlugin({
      ...
      generate: function generate(seed, files, entrypoints) {
        ...
        var entrypointFiles = entrypoints.main.filter(function (fileName) {
    ...
    useTypeScript && new ForkTsCheckerWebpackPlugin({
      ...
      reportFiles: [// This one is specifically to match during CI tests,
      // as micromatch doesn't match
      // '../cra-template-typescript/template/src/App.tsx'
      // otherwise.
      '../**/src/**/*.{ts,tsx}', '**/src/**/*.{ts,tsx}', '!**/src/**/__tests__/**', '!**/src/**/?(*.)(spec|test).*', '!**/src/setupProxy.*', '!**/src/setupTests.*'],
      silent: true,
      ...
      baseConfig: {
        ...
        rules: _objectSpread({}, !hasJsxRuntime && {
